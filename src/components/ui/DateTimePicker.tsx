...
const DateTimePicker: React.FC<DateTimePickerProps> = ({
  ...
  const isInRange = (date: Date) => {
    ...
    return (
      (isSameDay(d, s) || isAfter(d, s)) && (isSameDay(d, e) || isBefore(d, e))
    );
  };
  ...
  const isHovered = (date: Date) => {
    ...
    return (
      (isSameDay(d, s) || isAfter(d, s)) && (isSameDay(d, h) || isBefore(d, h))
    );
  };
    ...
    <div className="space-y-3">
      ...
      <Card className="p-4 bg-white shadow-md">
          ...
          <div className="flex items-center justify-between text-sm">
            <div className="flex-1">
              ...
              {!isMultipleDays && startDate && (
                <p className="text-accent text-xs font-semibold">{startTime}</p>
              )}
            </div>
            ...
            <div className="flex-1 text-right">
              ...
              {!isMultipleDays && startDate && (
                <p className="text-accent text-xs font-semibold">{endTime}</p>
              )}
            </div>
          </div>
          {getDuration() && (
            <div className="mt-2 pt-2 border-t border-accent/20">
              <p className="text-xs text-gray-600">
                Durée:{" "}
                <span className="font-bold text-accent">{getDuration()}</span>
              </p>
            </div>
          )}
        </div>
        ...
        <div className="mb-3 flex items-center justify-center">
          ...
          >
            {isMultipleDays
              ? "Plusieurs jours (9h-18h)"
              : "Journée avec horaires"}
          </button>
        </div>
