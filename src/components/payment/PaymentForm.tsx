...
const PaymentForm: React.FC<PaymentFormProps> = ({
  ...
  const processPaymentTransaction = async () => {
    ...
    try {
      ...
      const result = await processPayment(amount, paymentDetails, customerId);
  ...
  const renderSuccess = () => (
    <div className="text-center py-8">
      ...
      <p className="text-gray-600 mb-6">
        Votre paiement de {amount.toLocaleString()} {currency} a été traité avec
        succès.
      </p>
      {transactionId && (
      ...
      <Button onClick={() => onSuccess(transactionId || "")}>Continuer</Button>
    </div>
  );
