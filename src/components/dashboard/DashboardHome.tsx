...
const AdminDashboard = () => {
  ...
  const recentReservations = useMemo(() => {
    return reservations
      .sort(
        (a, b) =>
          new Date(b.dateCreation || b.createdAt).getTime() -
          new Date(a.dateCreation || a.createdAt).getTime(),
      )
      .slice(0, 5);
  }, [reservations]);
  ...
  const pendingDomiciliations = useMemo(() => {
    return demandesDomiciliation.filter((d) => d.statut === "en_attente")
      .length;
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          ...
          <Card className="p-6 bg-gradient-to-br from-blue-500 to-blue-600 text-white">
            <div className="flex items-center justify-between mb-4">
              ...
              {stats && stats.users.growth !== 0 && (
                <div
                  className={`flex items-center text-sm ${stats.users.growth > 0 ? "text-green-200" : "text-red-200"}`}
                >
          ...
          <Card className="p-6 bg-gradient-to-br from-green-500 to-green-600 text-white">
            <div className="flex items-center justify-between mb-4">
              ...
              {stats && stats.revenue.growth !== 0 && (
                <div
                  className={`flex items-center text-sm ${stats.revenue.growth > 0 ? "text-green-200" : "text-red-200"}`}
                >
        ...
        >
          <Card className="p-6 bg-gradient-to-br from-amber-500 to-amber-600 text-white">
            ...
            <p className="text-white/60 text-xs mt-2">
              {stats?.reservations.today || 0} aujourd'hui
            </p>
          </Card>
        </motion.div>
          ...
          <Card className="p-6 bg-gradient-to-br from-teal-500 to-teal-600 text-white">
            <div className="flex items-center justify-between mb-4">
              ...
              {stats && stats.occupancy.growth !== 0 && (
                <div
                  className={`flex items-center text-sm ${stats.occupancy.growth > 0 ? "text-green-200" : "text-red-200"}`}
                >
                  {stats.occupancy.growth > 0 ? (
                    <ArrowUp className="w-4 h-4" />
                  ) : (
                    <ArrowDown className="w-4 h-4" />
                  )}
            ...
            <p className="text-white/60 text-xs mt-2">
              {stats?.occupancy.occupied || 0}/{stats?.occupancy.total || 0}{" "}
              espaces
            </p>
          </Card>
        </motion.div>
              ...
              <div className="flex-1">
                ...
                <div className="space-y-2">
                  {pendingReservations > 0 && (
                    ...
                    >
                      <span>
                        {pendingReservations} reservation(s) en attente de
                        validation
                      </span>
                  ...
                  {pendingDomiciliations > 0 && (
                    <Link
                      to="/app/admin/domiciliations"
                      className="flex items-center justify-between text-sm text-amber-800 hover:text-amber-900"
                    >
                      <span>
                        {pendingDomiciliations} demande(s) de domiciliation en
                        attente
                      </span>
                      <ArrowUpRight className="w-4 h-4" />
                    </Link>
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <Link to="/app/reservations">
          <Card
            hover
            className="p-6 border-2 border-transparent hover:border-accent group transition-all"
          >
        ...
        <Link to="/app/domiciliation">
          <Card
            hover
            className="p-6 border-2 border-transparent hover:border-teal group transition-all"
          >
        ...
        <Link to="/app/profil">
          <Card
            hover
            className="p-6 border-2 border-transparent hover:border-warm group transition-all"
          >
            ...
            <p className="text-gray-600 text-sm mb-3">Gerer mes informations</p>
        ...
        <Link to="/app/mon-entreprise">
          <Card
            hover
            className="p-6 border-2 border-transparent hover:border-primary group transition-all"
          >
              ...
              >
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-600">
                      {formatDate(reservation.dateDebut)} a{" "}
...
const DashboardHome = () => {
  const { user } = useAuthStore();
  const { initializeData, loadUsers, loadDemandesDomiciliation } =
    useAppStore();
  const [loading, setLoading] = useState(true);
