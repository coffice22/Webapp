...
import { 
  FileText, Plus, Edit, Trash2, Eye, Download, Send, CreditCard, 
  Search, Filter, ArrowUpDown, CheckCircle, AlertCircle, Clock, 
  DollarSign, Calendar, Printer, Mail, Copy, RefreshCw
} from 'lucide-react';
...
const InvoiceManagement = () => {
  const { 
    ...
    members, getMemberById, generateInvoice, markInvoiceAsPaid
  } = useERPStore();
  ...
  const [newInvoice, setNewInvoice] = useState({
    memberId: '',
    items: [{ description: '', quantity: 1, unitPrice: 0, taxRate: 19, discount: 0 }],
    notes: '',
  ...
  const handleCreateInvoice = () => {
    ...
    if (newInvoice.items.length === 0 || newInvoice.items.some(item => !item.description || item.quantity <= 0)) {
      toast.error('Veuillez ajouter au moins un article valide');
      return;
    }
    
    const result = generateInvoice(newInvoice.memberId, newInvoice.items.map(item => ({
  ...
  const addInvoiceItem = () => {
    setNewInvoice(prev => ({
      ...prev,
      items: [...prev.items, { description: '', quantity: 1, unitPrice: 0, taxRate: 19, discount: 0 }]
    }));
  };
  ...
  const generatePDF = (invoice: Invoice) => {
    ...
    doc.text(`${member.billingAddress?.city || ''}, ${member.billingAddress?.postalCode || ''}`, 140, 50);
    ...
    const tableColumn = ['Description', 'Quantité', 'Prix unitaire', 'TVA', 'Remise', 'Total'];
    ...
    doc.text(formatCurrency(invoice.taxAmount), 170, finalY + 7, { align: 'right' });
    
    doc.text('Remise:', 130, finalY + 14);
    doc.text(formatCurrency(invoice.discount), 170, finalY + 14, { align: 'right' });
    
    doc.setFontSize(12);
    doc.setFont(undefined, 'bold');
    doc.text('Total:', 130, finalY + 24);
    doc.text(formatCurrency(invoice.total), 170, finalY + 24, { align: 'right' });
    ...
    doc.text('Coffice SARL - SIRET: 123456789 - TVA: FR12345678900', 105, 285, { align: 'center' });
  ...
  const exportInvoices = () => {
    const invoicesData = filteredInvoices.map(invoice => {
      ...
      return {
        ...
        client: member ? `${member.firstName} ${member.lastName}` : 'Client inconnu',
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-3xl font-display font-bold text-primary">Gestion des Factures</h1>
          <p className="text-gray-600">Gérez toutes les factures de votre coworking</p>
        </div>
        ...
        >
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 gap-4 p-4 bg-gray-50 rounded-lg">
              <div>
                ...
                <p className="font-medium">
                  {(() => {
                    ...
                    return member ? `${member.firstName} ${member.lastName}` : 'Client inconnu';
                  })()}
                </p>
              ...
              <div>
                <p className="text-sm text-gray-600 mb-1">Adresse de facturation</p>
                <p className="text-sm text-gray-700">
              ...
              <div className="bg-gray-50 rounded-lg overflow-hidden">
                <table className="min-w-full">
                  ...
                  <tfoot className="bg-gray-50">
                    ...
                    <tr>
                      <td colSpan={5} className="px-4 py-2 text-sm font-bold text-primary text-right">
                        Total
                      </td>
            ...
            {selectedInvoice.status === 'paid' && (
              <div className="p-4 bg-green-50 border border-green-200 rounded-lg">
                <div className="flex items-center">
                  ...
                  <div>
                    <p className="font-medium text-green-800">Payée le {formatDate(selectedInvoice.paidDate!)}</p>
                    <p className="text-sm text-green-700">
                      Méthode: {selectedInvoice.paymentMethod === 'cib' ? 'Carte CIB' : 
                               selectedInvoice.paymentMethod === 'dahabia' ? 'Carte Dahabia' : 
        ...
        <div className="space-y-6">
            ...
            >
              ...
              {members.map(member => (
                <option key={member.id} value={member.id}>
                  {member.firstName} {member.lastName} {member.companyName ? `(${member.companyName})` : ''}
                </option>
              ))}
            </select>
          </div>
              ...
              {newInvoice.items.map((item, index) => (
                <div key={index} className="p-4 bg-gray-50 rounded-lg">
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="text-right">
                      <p className="text-sm text-gray-600">Total: {formatCurrency((item.quantity * item.unitPrice) - (item.discount || 0))}</p>
                    </div>
                  </div>
                </div>
              ))}
          ...
          <div className="p-4 bg-accent/5 border border-accent/20 rounded-lg">
            ...
            <div className="space-y-2">
              <div className="flex justify-between">
                ...
                <span className="font-medium">{formatCurrency(calculateSubtotal(newInvoice.items))}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-600">TVA:</span>
                <span className="font-medium">{formatCurrency(calculateTax(newInvoice.items))}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-600">Remise:</span>
                <span className="font-medium">{formatCurrency(calculateDiscount(newInvoice.items))}</span>
              </div>
              <div className="border-t border-gray-200 pt-2 mt-2">
                <div className="flex justify-between">
                  <span className="font-medium text-primary">Total:</span>
                  <span className="font-bold text-accent">{formatCurrency(calculateTotal(newInvoice.items))}</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="flex space-x-4 pt-4">
            ...
            <Button
              ...
              className="flex-1"
              disabled={!newInvoice.memberId || newInvoice.items.some(item => !item.description || item.quantity <= 0)}
            >
              Créer la facture
            </Button>
          </div>
          ...
          <div className="space-y-6">
            <div className="p-4 bg-gray-50 rounded-lg">
              <div className="flex justify-between items-center">
                <div>
                  <h3 className="font-medium text-primary mb-1">Facture #{selectedInvoice.number}</h3>
                ...
                <div className="text-right">
                  ...
                  <p className="text-xl font-bold text-accent">{formatCurrency(selectedInvoice.total)}</p>
                </div>
              </div>
            </div>
