...
const ERPSystem = () => {
  ...
  const handleCreateEspace = async () => {
    const result = await addEspace({
      ...
      equipements:
        formData.equipements?.split(",").map((e: string) => e.trim()) || [],
  ...
  const renderReports = () => {
    ...
    const reservationsAujourdhui = reservations.filter(
      (r) => new Date(r.dateDebut).toDateString() === today.toDateString(),
    ).length;
    ...
    const ticketMoyen =
      stats.reservationsCeMois > 0
        ? stats.caMois / stats.reservationsCeMois
        : 0;

    const abonnesActifs = abonnementsUtilisateurs.filter(
      (a) => a.statut === "actif",
    ).length;
    ...
    const espacePlusReserve = espaces.reduce(
      (max, espace) => {
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          <Card className="p-5 bg-gradient-to-br from-green-50 to-white border-green-200">
            <div className="flex items-center justify-between mb-3">
              ...
              <div
                className={`flex items-center text-sm font-semibold ${(typeof croissanceCA === "number" && croissanceCA >= 0) || (typeof croissanceCA === "string" && parseFloat(croissanceCA) >= 0) ? "text-green-600" : "text-red-600"}`}
              >
                <TrendingUp className="w-4 h-4 mr-1" />
                {(typeof croissanceCA === "number"
                  ? croissanceCA
                  : parseFloat(croissanceCA)) > 0
                  ? "+"
                  : ""}
                {croissanceCA}%
              </div>
            ...
            <p className="text-3xl font-bold text-gray-900 mb-1">
              {formatCurrency(stats.caMois, "DZD")}
            </p>
            <p className="text-sm text-gray-600">CA du mois</p>
            <p className="text-xs text-gray-500 mt-2">
              vs {formatCurrency(caMoisDernier, "DZD")} mois dernier
            </p>
          </Card>
          ...
          <Card className="p-5 bg-gradient-to-br from-blue-50 to-white border-blue-200">
            ...
            <p className="text-3xl font-bold text-gray-900 mb-1">
              {stats.reservationsCeMois}
            </p>
            <p className="text-sm text-gray-600">Réservations ce mois</p>
            <p className="text-xs text-gray-500 mt-2">
              {heuresReservees.toFixed(0)}h réservées
            </p>
          </Card>
          ...
          <Card className="p-5 bg-gradient-to-br from-accent/10 to-white border-accent/30">
            <div className="flex items-center justify-between mb-3">
              ...
              <Badge style={{ backgroundColor: "#14B8A6" }}>
                +{nouveauxUtilisateurs} nouveau(x)
              </Badge>
            </div>
            <p className="text-3xl font-bold text-gray-900 mb-1">
              {stats.activeUsers}
          ...
          <Card className="p-5 bg-gradient-to-br from-teal-50 to-white border-teal-200">
            ...
            <p className="text-xs text-gray-500 mt-2">
              {espaces.filter((e) => e.disponible).length}/{espaces.length}{" "}
              espaces dispo
            </p>
          </Card>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="flex items-center mb-3">
              ...
              <div>
                <p className="text-2xl font-bold text-gray-900">
                  {formatCurrency(ticketMoyen, "DZD")}
                </p>
          ...
          <Card className="p-5">
            ...
            <p className="text-xs text-gray-500">
              Revenus: {formatCurrency(revenus_abonnements, "DZD")}/mois
            </p>
          </Card>
        ...
        <Card className="p-6">
          ...
          <div className="space-y-4">
            {[
              ...
              const reservationsType = reservations.filter(
                (r) => r.espace?.type === type,
              );
              const caType = reservationsType.reduce(
              ...
              const percentage =
                reservations.length > 0
                  ? (
                      (reservationsType.length / reservations.length) *
                      100
                    ).toFixed(0)
                  : 0;
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-4">
                      <span className="text-sm text-gray-600">
                        {reservationsType.length} reservations
                      </span>
                      <span className="text-sm font-semibold text-green-600">
                        {formatCurrency(caType, "DZD")}
                      </span>
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between p-4 bg-green-50 rounded-lg">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xl font-bold text-gray-900">
                      {formatCurrency(
                        stats.caMois - revenus_abonnements,
                        "DZD",
                      )}
                    </p>
                  </div>
                ...
                <div className="text-2xl font-bold text-green-600">
                  {caTotal > 0
                    ? (
                        ((stats.caMois - revenus_abonnements) / caTotal) *
                        100
                      ).toFixed(0)
                    : 0}
                  %
              ...
              <div className="flex items-center justify-between p-4 bg-blue-50 rounded-lg">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xl font-bold text-gray-900">
                      {formatCurrency(revenus_abonnements, "DZD")}
                    </p>
                  </div>
                </div>
                <div className="text-2xl font-bold text-blue-600">
                  {caTotal > 0
                    ? ((revenus_abonnements / caTotal) * 100).toFixed(0)
                    : 0}
                  %
                </div>
              </div>
                  ...
                  >
                    ...
                    <div className="text-right">
                      <p className="font-bold text-green-600">
                        {formatCurrency(u.totalSpent, "DZD")}
                      </p>
                      <p className="text-xs text-gray-500">
                        {
                          reservations.filter((r) => r.utilisateur?.id === u.id)
                            .length
                        }{" "}
                        rés.
                      </p>
                  ...
                  transactions.slice(0, 10).map((tx) => (
                    <tr key={tx.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-4 py-3">
                        <div>
                          <p className="font-medium text-gray-900">
                            {tx.utilisateur.prenom} {tx.utilisateur.nom}
                          </p>
                          <p className="text-sm text-gray-500">
                            {tx.utilisateur.email}
                          </p>
                        </div>
                      </td>
                      <td className="px-4 py-3">
                      ...
                      <td className="px-4 py-3">
                        <span
                          className={`font-semibold ${tx.type === "remboursement" ? "text-red-600" : "text-green-600"}`}
                        >
                          {tx.type === "remboursement" ? "-" : "+"}{" "}
                          {formatCurrency(tx.montant, "DZD")}
                        </span>
                      </td>
                      <td className="px-4 py-3">
                        <Badge
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <Input
                  ...
                  onChange={(e) =>
                    setFormData({ ...formData, dureeJours: e.target.value })
                  }
                  placeholder="30"
                />
                <div>
