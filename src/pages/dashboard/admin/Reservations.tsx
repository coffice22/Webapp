...
const Reservations = () => {
  ...
  const handleStatusChange = async (id: string, statut: string) => {
    try {
      ...
      toast.success(
        `Réservation ${statut === "confirmee" ? "confirmée" : "refusée"}`,
      );
    } catch (error) {
  ...
  const handleBulkAction = async (action: "confirmer" | "refuser") => {
    ...
    try {
      ...
      const results = await Promise.all(
        selectedIds.map((id) =>
          updateReservation(id, { statut: statut as any }),
        ),
      );
      ...
      if (failed.length > 0) {
        toast.error(
          `${failed.length} réservation(s) n'ont pas pu être mises à jour`,
        );
        return;
      }

      toast.success(
        `${selectedIds.length} réservation(s) ${action === "confirmer" ? "confirmées" : "refusées"}`,
      );
      setSelectedIds([]);
    } catch (error) {
    ...
    return reservations
      .filter((res) => {
        const matchSearch =
          ...
          res.utilisateur?.prenom
            ?.toLowerCase()
            .includes(searchTerm.toLowerCase()) ||
          res.espace?.nom?.toLowerCase().includes(searchTerm.toLowerCase());

        const matchStatut =
          statutFilter === "tous" || res.statut === statutFilter;
        const matchEspace =
          espaceFilter === "tous" || res.espace?.id === espaceFilter;
                  ...
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1 grid grid-cols-1 md:grid-cols-4 gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-gray-500">
                          {res.participants} participant(s)
                        </p>
                      </div>
                      ...
                      {res.statut === "en_attente" && (
                        <div className="flex gap-2">
                          <Button
                            ...
                            variant="success"
                            onClick={() =>
                              handleStatusChange(res.id, "confirmee")
                            }
                          >
                          ...
                          <Button
                            ...
                            variant="danger"
                            onClick={() =>
                              handleStatusChange(res.id, "annulee")
                            }
                          >
